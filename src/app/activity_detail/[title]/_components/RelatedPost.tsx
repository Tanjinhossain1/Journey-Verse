...
import { LoveStatusType } from "./ActivityDisplay";
...
export default function RelatedActivity({
  ...
  const [activity, setActivity] = React.useState<ActivityTypes[]>([]);
  ...
  React.useEffect(() => {
    const fetchArticles = async () => {
      const articles = await getPaginatedActivity(1, 12, undefined, title);
      setActivity(articles.data as ActivityTypes[]);
    };
    fetchArticles();
  }, []);
  ...
  const getPages = () => {
    ...
    for (let i = 0; i < totalPages; i++) {
      ...
      pages.push(activity.slice(start, end));
    }
    return pages;
  };
                  ...
                  {page.map((activity) => (
                    ...
                    >
                      <div className="relative overflow-hidden group">
                        ...
                        loveReact.some(
                            (love) => love.hotel_name === activity.title
                          ) ? (
                          <button
